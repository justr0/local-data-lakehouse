...
start_services() {
    ...
    docker compose -f docker-compose-dremio.yaml up -d
    sleep 30
...
load_superset_drivers() {
    ...
    docker exec -it superset pip install trino sqlalchemy_dremio snowflake-sqlalchemy
    
    echo "Trino db drivers loaded into Superset"
